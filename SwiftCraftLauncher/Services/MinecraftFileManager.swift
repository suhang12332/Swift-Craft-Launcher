...
class MinecraftFileManager {
    ...
    ///   - gameName: 游戏名称
    /// - Returns: 是否成功下载
    func downloadVersionFiles(manifest: MinecraftVersionManifest, gameName: String) async -> Bool {
        do {
            try await downloadVersionFilesThrowing(manifest: manifest, gameName: gameName)
            return true
        } catch {
            let globalError = GlobalError.from(error)
            Logger.shared.error("下载 Minecraft 版本文件失败: \(globalError.chineseMessage)")
    ...
    func downloadVersionFilesThrowing(manifest: MinecraftVersionManifest, gameName: String) async throws {
            ...
            if !fileManager.fileExists(atPath: directory.path) {
                do {
                    ...
                    throw GlobalError.fileSystem(
                        chineseMessage: "创建目录失败: \(directory.path), 错误: \(error.localizedDescription)",
    ...
    private func downloadLibrary(_ library: Library, metaDirectory: URL) async throws {
        let destinationURL = metaDirectory.appendingPathComponent("libraries").appendingPathComponent(library.downloads.artifact.path)
        
        do {
            _ = try await DownloadManager.downloadFile(urlString: library.downloads.artifact.url!.absoluteString, destinationURL: destinationURL, expectedSha1: library.downloads.artifact.sha1)
            ...
            let globalError = GlobalError.from(error)
            throw GlobalError.download(
                chineseMessage: "下载库文件失败 \(library.name): \(globalError.chineseMessage)",
    ...
    private func downloadNativeLibrary(library: Library, classifiers: [String: LibraryArtifact], metaDirectory: URL) async throws {
        #if os(macOS)
    ...
    private func downloadAndSaveFile(from url: URL, to destinationURL: URL, sha1: String?, fileNameForNotification: String? = nil, type: DownloadType) async throws {
        ...
        do {
            ...
            throw GlobalError.fileSystem(
                chineseMessage: "创建目录失败: \(destinationURL.deletingLastPathComponent().path), 错误: \(error.localizedDescription)",
                i18nKey: "error.filesystem.directory_creation_failed",
                level: .notification
            )
        }
        ...
        do {
            ...
            if let expectedSha1 = sha1 {
                ...
                if downloadedSha1 != expectedSha1 {
                    throw GlobalError.download(
                        chineseMessage: "SHA1 校验失败: 期望 \(expectedSha1), 实际 \(downloadedSha1)",
    ...
    private func downloadAsset(asset: AssetIndexData.AssetObject, path: String, objectsDirectory: URL) async throws {
        ...
        do {
            _ = try await DownloadManager.downloadFile(urlString: "https://resources.download.minecraft.net/\(String(asset.hash.prefix(2)))/\(asset.hash)", destinationURL: destinationURL, expectedSha1: asset.hash)
            incrementCompletedFilesCount(fileName: String(format: "file.asset".localized(), path), type: .resources)
        } catch {
            let globalError = GlobalError.from(error)
            throw GlobalError.download(
                chineseMessage: "下载资源文件失败 \(path): \(globalError.chineseMessage)",
