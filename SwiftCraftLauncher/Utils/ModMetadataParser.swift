...
class ModMetadataParser {
    ...
    static func parseModMetadataThrowing(fileURL: URL) throws -> (String?, String?) {
        ...
        do {
            ...
            throw GlobalError.validation(
                chineseMessage: "无法打开压缩包: \(fileURL.lastPathComponent), 错误: \(error.localizedDescription)",
        ...
        if let entry = archive["META-INF/mods.toml"] {
            if let (modid, version) = try parseForgeTomlThrowing(archive: archive, entry: entry) {
        ...
        if let entry = archive["fabric.mod.json"] {
            if let (modid, version) = try parseFabricJsonThrowing(archive: archive, entry: entry) {
        ...
        if let entry = archive["mcmod.info"] {
            if let (modid, version) = try parseMcmodInfoThrowing(archive: archive, entry: entry) {
    ...
    private static func parseForgeTomlThrowing(archive: Archive, entry: Entry) throws -> (String, String)? {
        var data = Data()
        do {
            ...
            throw GlobalError.validation(
                chineseMessage: "提取 mods.toml 失败: \(error.localizedDescription)",
    ...
    private static func parseFabricJsonThrowing(archive: Archive, entry: Entry) throws -> (String, String)? {
        var data = Data()
        do {
            try _ = archive.extract(entry) { chunk in
                data.append(chunk)
            ...
            throw GlobalError.validation(
                chineseMessage: "提取 fabric.mod.json 失败: \(error.localizedDescription)",
        ...
        do {
            ...
            throw GlobalError.validation(
                chineseMessage: "解析 fabric.mod.json 失败: \(error.localizedDescription)",
                i18nKey: "error.validation.fabric_mod_json_parse_failed",
                level: .silent
            )
        }
        
        guard let modid = json["id"] as? String, let version = json["version"] as? String else {
            throw GlobalError.validation(
    ...
    private static func parseMcmodInfoThrowing(archive: Archive, entry: Entry) throws -> (String, String)? {
        var data = Data()
        do {
            try _ = archive.extract(entry) { chunk in
                data.append(chunk)
            ...
            throw GlobalError.validation(
                chineseMessage: "提取 mcmod.info 失败: \(error.localizedDescription)",
        ...
        do {
            ...
            throw GlobalError.validation(
                chineseMessage: "解析 mcmod.info 失败: \(error.localizedDescription)",
        ...
        guard let modid = first["modid"] as? String, let version = first["version"] as? String else {
            throw GlobalError.validation(
