...
class GameLogCollector {
    ...
    private func openAIWindowWithLogs(logFiles: [URL], gameName: String) async {
        ...
        // 构建消息内容
        let messageContent = buildMessageContent(logFiles: logFiles, gameName: gameName)
        ...
        // 发送消息和附件
        await AIChatManager.shared.sendMessage(messageContent, attachments: attachments, chatState: chatState)
    }
    ...
    ///   - gameName: 游戏名称
    /// - Returns: 消息内容
    private func buildMessageContent(logFiles: [URL], gameName: String) -> String {
        let fileCount = logFiles.count
        let isCrashReport = logFiles.first?.path.contains("crash-reports") ?? false
        
        if isCrashReport {
            return "请帮我分析这个游戏的崩溃报告。游戏名称：\(gameName)，共 \(fileCount) 个崩溃报告文件。"
        } else {
            return "请帮我分析这个游戏的日志文件。游戏名称：\(gameName)，日志文件：latest.log。"
        }
    }
}
