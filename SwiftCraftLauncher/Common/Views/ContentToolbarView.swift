...
//            }
    ...
    private func checkAnnouncement() async {
        ...
        if let cachedData = AnnouncementCache.cachedData,
           ...
           Date().timeIntervalSince(lastCheck) < AnnouncementCache.cacheDuration {
            await MainActor.run {
                self.hasAnnouncement = true
                self.announcementData = cachedData
            }
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let data = data {
                    ...
                    // 更新缓存
                    AnnouncementCache.cachedData = data
                    AnnouncementCache.lastCheckTime = Date()
                } else {
                    self.hasAnnouncement = false
                    self.announcementData = nil
                    // 清除缓存
                    AnnouncementCache.cachedData = nil
                    AnnouncementCache.lastCheckTime = Date()
                }
            }
        } catch {
