...
struct EasyTierDownloadProgressWindow: View {
    @ObservedObject var downloadState: EasyTierDownloadState
    @Environment(\.dismiss)
    private var dismiss
    ...
    var body: some View {
        ...
        VStack {
            if downloadState.hasError {
                // 错误状态 要显示重试按钮
                EasyTierDownloadItemView(
                    icon: "exclamationmark.triangle.fill",
                    title: downloadState.title,
                    subtitle: downloadState.errorMessage,
                    status: .error,
                    onCancel: {
                        EasyTierDownloadManager.shared.retryDownload()
                    },
                ...
                EasyTierDownloadItemView(
                    ...
                    onCancel: {
                        EasyTierDownloadManager.shared.cancelDownload()
                    },
                    downloadState: downloadState
                )
            }
        ...
        .onAppear {
            // 设置窗口关闭回调
            EasyTierDownloadManager.shared.setDismissCallback {
                dismiss()
            }
        }
...
// 下载项视图（复用 Java 下载的视图组件结构）
struct EasyTierDownloadItemView: View {
    let icon: String
    let title: String
    let subtitle: String
    let status: DownloadStatus
    let onCancel: () -> Void
    let downloadState: EasyTierDownloadState?
