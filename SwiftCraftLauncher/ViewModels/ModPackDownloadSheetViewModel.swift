...
class ModPackDownloadSheetViewModel: ObservableObject {
   ...
   func downloadGameIcon(
       ...
       do {
           ...
           // 获取游戏目录
           guard let gameDirectory = AppPaths.profileDirectory(gameName: gameName) else {
               handleDownloadError("无法获取游戏目录: \(gameName)", "error.configuration.game_directory_not_found")
               return nil
           }
